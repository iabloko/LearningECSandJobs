...
namespace Mynamespace {
    public class SpawnCapsuleSystem : ComponentSystem {
#pragma warning disable 649
        struct Group {
            [ReadOnly]
            public SharedComponentDataArray<SpawnCapsule> Spawner;
        ...
        protected override void OnUpdate () {
            while (m_Group.Length != 0) {
                ...
                if (spawner.spawnLocal) {
                    //GeneratePoints.RandomPointsOnCircle (new float3 (), spawner.count, ref positions);
                    GeneratePoints._awake (ref positions);
                    ...
                    GeneratePoints.RandomPointsOnCircle (center, spawner.count, ref positions);
                    for (int i = 0; i < spawner.count; i++) {
                        var position = new Position {
                            Value = positions[i]
                        };
                ...
                EntityManager.RemoveComponent<SpawnCapsule> (sourceEntity);
